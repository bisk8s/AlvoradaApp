...
export const getShadow = (elevation: number) => {
  const _elevation = Math.min(24, Math.max(0, elevation - 1));
  const s = parseShadow(androidDepth.penumbra[_elevation]);
...
export const defaultShadow = getShadow(8);

export default { styles: GlobalStyles, getShadow };
